...
class FewShotBase:
    def __init__(self, dataset_name: str, num_classes: int, pretrain_model):
        ...
        self.fewshot_labels: torch.Tensor = torch.load(f"data/fewshot_{dataset_name}_graph/testset/labels.pt").cuda()
        ...
        self.fewshot_one_hot_label = torch.nn.functional.one_hot(self.fewshot_labels.long(), num_classes=num_classes).float().cuda()
